...
Running `rubocop --json-schema` outputs a JSON Schema that Intellij can use to enable https://www.jetbrains.com/help/idea/json.html[validation and autocompletion] within your `.rubocop.yml` file.
...
The https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml[yaml] extension allows validation and autocompletion of your `.rubocop.yml` file itself. Run `rubocop --json-schema > .vscode/rubocop_schema.json` (or similar) to generate a JSON Schema file, and then tell your editor to use it by adding something like the following to your workspace settings:

[source,json]
----
"yaml.schemas": {
  ".vscode/rubocop_schema.json": [".rubocop.yml", ".rubocop_todo.yml"]
}
----

If you're using regular expressions or other Ruby objects in your `.rubocop.yml`, you may also need a line like this:

[source,json]
----
"yaml.customTags": ["!ruby/regexp scalar"]
----
